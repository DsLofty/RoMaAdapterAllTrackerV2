...
class KubricMovifDataset(PointDataset):
    def __init__(
        ...
        self.query_rich_crop = query_rich_crop
        self.query_rich_topk = max(1, int(query_rich_topk))
    ...
    def sample_query_rich_start(self, visibs, seq_len, frame_rate):
        span = int(seq_len) * int(frame_rate)
        max_start = int(len(visibs)) - span
        if max_start <= 0:
            return 0
        starts = np.arange(max_start + 1, dtype=np.int64)
        counts = np.asarray(visibs[starts]).astype(bool).sum(axis=1)
        if counts.size == 0 or int(counts.max()) <= 0:
            return int(np.random.choice(starts))
        topk = min(int(self.query_rich_topk), int(counts.shape[0]))
        if topk < int(counts.shape[0]):
            candidate_ids = np.argpartition(-counts, topk - 1)[:topk]
        else:
            candidate_ids = np.arange(counts.shape[0])
        candidate_starts = starts[candidate_ids]
        return int(np.random.choice(candidate_starts))
    ...
    def getitem_helper(self, index):
        ...
        seq_name = os.path.relpath(fol, self.data_root).replace(os.sep, '_')
        ...
        if "point_xys_valid" in annot_dict:
            valids = annot_dict["point_xys_valid"]
        elif "valids" in annot_dict:
            valids = annot_dict["valids"]
        elif "point_valids" in annot_dict:
            valids = annot_dict["point_valids"]
        else:
            trajs64 = trajs.astype(np.float64)
            valids = np.isfinite(trajs64).all(axis=-1)
            valids = valids & (np.sum(np.abs(trajs64), axis=-1) < 100000)
        valids = np.asarray(valids).astype(bool)
        if valids.shape != visibs.shape:
            raise ValueError(
                "valid mask shape %s does not match visibility shape %s for %s"
                % (str(valids.shape), str(visibs.shape), str(npy_path))
            )
        ...
        if seq_len < len(rgbs):
            ...
            if self.query_rich_crop and not self.random_first_frame:
                start_ind = self.sample_query_rich_start(visibs, seq_len, frame_rate)
            elif seq_len * frame_rate < len(rgbs):
